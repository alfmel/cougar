 ...
 * 2014.03.22:
 *   (AT)  Added authorizationHeader method
 *   (AT)  Get headers in the constructor and avoid future call to headers
 * 2014.03.24:
...
class RestService implements iRestService
{
    ...
    public function __construct()
    {
        ...
        # Parse the headers now (avoid very probable call to headers() later)
        ...
        else
        {
            ...
            # Add the content type (found somewhere else)
        ...
        # Create the header map (maps lower-case header names to actual names)
        ...
        if (count($_POST) == 0)
        {
            ...
            if ($_METHOD != "POST")
            {
                # Get the content-type header (done inline for performance)
                if (array_key_exists("content-type", $this->headerMap))
                {
                    # See if the Content-Type header is a url-encoded form
                    if (strpos($this->headers[$this->headerMap["content-type"]],
     ...
     *   (AT)  Initial release
     * 2014.03.22:
     *   (AT)  Return stored headers previously parsed by constructor
     *
     * @version 2014.03.22
     * @author (AT) Alberto Trevino, Brigham Young Univ. <alberto@byu.edu>
     * 
     ...
     *         from the list
     * 2014.03.22:
     *   (AT)  Simplify extraction now that headers are extracted in the
     *         constructor
     *
     * @version 2014.03.06
     * @author (AT) Alberto Trevino, Brigham Young Univ. <alberto@byu.edu>
     ...
     * values:
     *
     *   rawHeader    The full, raw header (scheme and parameter)
    ...
    {
        // See if we have the authorization header in our headers array
        // if (array_key_exists("authorization", $this->headerMap))
        if ($this->header("Authorization"))
        {
            // Prepare the response
            $response = array("rawHeader" =>
                #$this->headers[$this->headerMap["authorization"]],
                $this->header("Authorization"),
                "scheme" => "",
                "parameter" => "");

            // Separate the scheme and the parameter from the raw header
            $values = preg_split('/\s+/u', $response["rawHeader"], 2);
            $response["scheme"] = $values[0];
            $response["parameter"] = $values[1];

            // See if PHP already parsed username and password
            if (array_key_exists("PHP_AUTH_USER", $_SERVER))
            {
                // PHP already parsed these for us
                $response["username"] = $_SERVER["PHP_AUTH_USER"];
                $response["password"] = $_SERVER["PHP_AUTH_PW"];
            }
        }
        // See if PHP parsed the Basic authentication values already
        else if (array_key_exists("PHP_AUTH_USER", $_SERVER))
        {
            // Create the response, recreating the raw parameter as well
            $parameter = base64_encode($_SERVER["PHP_AUTH_USER"] . ":" .
                $_SERVER["PHP_AUTH_PW"]);

            $response = array("rawHeader" => "Basic " . $parameter,
                "scheme" => "Basic",
                "parameter" => $parameter,
        ...
        {
            // We don't seem to have a header; return an empty response
            $response = null;
        }
     ...
     * 2014.03.22:
     *   (AT)  Access headers directly now that they are parsed by constructor
     *
     * @version 2014.03.22
     * @author (AT) Alberto Trevino, Brigham Young Univ. <alberto@byu.edu>
     * 
